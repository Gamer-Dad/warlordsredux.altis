...
if (_side != playerSide) exitWith {
	if (_selected) then {
		...
		if ((_sector getVariable "BIS_WL_owner") == playerSide || {_sector == WL_TARGET_FRIENDLY}) then {
		...
		if (_sector == BIS_WL_playerBase) then {
			...
			if !(isServer) then {
				["base_vulnerable", playerSide] call BIS_fnc_WL2_handleRespawnMarkers;
			};
		};
		...
		_sector spawn {
			...
			waitUntil {sleep WL_TIMEOUT_STANDARD; playerSide in (_sector getVariable ["BIS_WL_revealedBy", []]) || {WL_TARGET_ENEMY != _sector}};
		...
		if ((markerPos "BIS_WL_targetEnemy") distance2D BIS_WL_playerBase < 1 && {!isServer}) then {
			["base_safe", playerSide] call BIS_fnc_WL2_handleRespawnMarkers;
		};
	};
};
...
if (_selected) then {
	...
	_sector spawn {
		params ["_sector"];
		_t = serverTime + 3;
		waitUntil {serverTime > _t || {playerSide in (_sector getVariable "BIS_WL_previousOwners")}};
		if (!(playerSide in (_sector getVariable "BIS_WL_previousOwners"))) then {
