...
MRTM_fnc_iconDrawMap = {
	...
	} forEach ((entities [["Tank", "Car", "Plane", "Helicopter"], ["Logic"], false, true]) select {((side (crew _x select 0)) == (side group player)) && {(alive _x) && {(typeOf _x != "B_Truck_01_medical_F") && {(typeOf _x != "O_Truck_03_medical_F")}}}});
	{
		if (!isNull _x) then {
			_m drawIcon [
				[_x] call MRTM_fnc_iconType,
