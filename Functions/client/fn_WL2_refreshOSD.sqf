...
(uiNamespace getVariable "BIS_WL_osd_income_side_2") ctrlSetStructuredText parseText format ["<t size = '%3' shadow = '2'>%1/%2</t>", ((((missionNamespace getVariable [format ["BIS_WL_maxSubordinates_%1", side group player], 1]) - count ((units player) select {_x getVariable ["BIS_WL_ownerAsset", "123"] == getPlayerUID player})) + 1) max 0), (missionNamespace getVariable [format ["BIS_WL_maxSubordinates_%1", side group player], 1]), 0.65 call BIS_fnc_WL2_sub_purchaseMenuGetUIScale];
