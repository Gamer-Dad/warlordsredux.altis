...
} forEach (BIS_WL_allWarlords select {isPlayer _x && side group _x == side group player && _x != player});
...
[_purchase_transfer_units, _purchase_transfer_ok, _purchase_transfer_amount] spawn {
	...
	while {ctrlEnabled (_this # 1)} do {
		...
		lbClear (_this # 0);
		{(_this # 0) lbAdd name _x} forEach (BIS_WL_allWarlords select {isPlayer _x && side group _x == side group player && _x != player});
		lbSort (_this # 0);
