...
{_purchase_transfer_units lbAdd (format ["%1 - %2", name _x, ((missionNamespace getVariable "fundsDatabaseClients") get (getPlayerUID _x))])} forEach ((BIS_WL_allWarlords select {isPlayer _x && side group _x == side group player}) - [player]);
...
[_purchase_transfer_units, _purchase_transfer_ok, _purchase_transfer_amount] spawn {
	...
	while {ctrlEnabled (_this # 1)} do {
		...
		lbClear (_this # 0);
		{(_this # 0) lbAdd (format ["%1 - %2", name _x, ((missionNamespace getVariable "fundsDatabaseClients") get (getPlayerUID _x))])} forEach ((BIS_WL_allWarlords select {isPlayer _x && side group _x == side group player}) - [player]);
		lbSort (_this # 0);
