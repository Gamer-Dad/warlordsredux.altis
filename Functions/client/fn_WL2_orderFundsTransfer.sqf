...
{_purchase_transfer_units lbAdd name _x} forEach ((allPlayers select {isPlayer _x && {side group _x == BIS_WL_playerSide}}) - [player]);
...
[_purchase_transfer_units, _purchase_transfer_ok, _purchase_transfer_amount] spawn {
	...
	while {ctrlEnabled (_this # 1)} do {
		...
		lbClear (_this # 0);
		{(_this # 0) lbAdd name _x} forEach ((allPlayers select {isPlayer _x && {side group _x == BIS_WL_playerSide}}) - [player]);
		lbSort (_this # 0);
