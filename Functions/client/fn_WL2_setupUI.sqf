		...
		_purchase_request ctrlAddEventHandler ["ButtonClick", {
			if (uiNamespace getVariable ["BIS_WL_purchaseMenuItemAffordable", FALSE]) then {
				...
				switch (_className) do {
					case "Arsenal": {if (isNull (findDisplay 602)) then {["RequestMenu_close"] spawn BIS_fnc_WL2_setupU; [player, "orderArsenal", BIS_WL_arsenalCost, [], player] remoteExec ["BIS_fnc_WL2_handleClientRequest", 2]} else {playSound "AddItemFailed"}};
					case "LastLoadout": {["RequestMenu_close"] spawn BIS_fnc_WL2_setupU; [player, "lastLoadout", BIS_WL_lastLoadoutCost, [], player] remoteExec ["BIS_fnc_WL2_handleClientRequest", 2]};
					case "SaveLoadout": {"save" call BIS_fnc_WL2_orderSavedLoadout};
					case "SavedLoadout": {["RequestMenu_close"] spawn BIS_fnc_WL2_setupU; [player, "savedLoadout", BIS_WL_savedLoadoutCost, [], player] remoteExec ["BIS_fnc_WL2_handleClientRequest", 2]};
					...
					case "TargetReset": {["RequestMenu_close"] spawn BIS_fnc_WL2_setupU; [player, "targetReset", 500, [0,0,0], 0, false] remoteExec ["BIS_fnc_WL2_handleClientRequest", 2]};
