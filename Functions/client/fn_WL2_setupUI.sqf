...
switch (_displayClass) do {
	...
	case "RequestMenu_open": {
		...
		_myDisplay spawn {
			_selectedCnt = count ((groupSelectedUnits player) select {_x != player});
			while {!isNull _this} do {
				waitUntil {sleep WL_TIMEOUT_MIN; count ((groupSelectedUnits player) select {_x != player}) != _selectedCnt};
				_selectedCnt = count ((groupSelectedUnits player) select {_x != player});
		...
		_purchase_request ctrlAddEventHandler ["ButtonClick", {
			if (uiNamespace getVariable ["BIS_WL_purchaseMenuItemAffordable", FALSE]) then {
				...
				switch (_className) do {
					...
					case "RemoveUnits": {{_x call BIS_fnc_WL2_sub_deleteAsset} forEach ((groupSelectedUnits player) select {_x != player})};
