...
{_purchase_bounty_units lbAdd name _x} forEach ((BIS_WL_allWarlords select {(isPlayer _x) && ((_x getVariable [format ["BIS_WL_Bounty_%1", getPlayerUID _x], "None"]) != "None")}) - [player]);
...
[_purchase_bounty_units, _purchase_bounty_ok, _purchase_bounty_amount] spawn {
	...
	while {ctrlEnabled (_this # 1)} do {
		...
		lbClear (_this # 0);
		{(_this # 0) lbAdd name _x} forEach ((BIS_WL_allWarlords select {(isPlayer _x) && ((_x getVariable [format ["BIS_WL_Bounty_%1", getPlayerUID _x], "None"]) != "None")}) - [player]);
		lbSort (_this # 0);
