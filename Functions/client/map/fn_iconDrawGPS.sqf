...
} forEach ((allUnits) select {(side group (crew _x select 0) == _side) && {(alive _x) && {(isNull objectParent _x) && {(typeOf _x != "Logic") && {_x != player}}}}});
{
	_size = call BIS_fnc_iconSize;
	_m drawIcon [
		call BIS_fnc_iconType,
