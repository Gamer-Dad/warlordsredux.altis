...
BIS_WL_assetMapHandler = addMissionEventHandler ["EachFrame", {
	...
	if (visibleMap) then {
		_nearbyAssets = ((WL_CONTROL_MAP ctrlMapScreenToWorld getMousePosition) nearObjects ["All", (((ctrlMapScale WL_CONTROL_MAP) * 500) min 50) max 2]) select {!isNull (_x getVariable ["BIS_WL_ownerAsset", objNull])};
