...
while {dialog} do {
	...
	switch (_curSel) do {
		...
		case "changelog": {
			...
			"-Custom keybind lock/unlock, Custom action 1", lineBreak,
			"-Custom keybind earplugs, Custom action 2", lineBreak,
			"-Anti cheat against cheaters killing all players / teleporting players improved", lineBreak,
			"", lineBreak,
			"2.5.4", lineBreak,
			"-option to disable kill sound", lineBreak,
			"-1st person kill bonus", lineBreak,
			"-Volume slider for RWR", lineBreak,
			"-New settings menu", lineBreak,
			"-Voice warning system improved", lineBreak,
			"-Friendly AI shown on map", lineBreak,
			"-Crew of vehicle all get an assist", lineBreak,
			"-Kick players out of vehicle", lineBreak,
