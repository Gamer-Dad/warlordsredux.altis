							...
							"",
							{
								...
								if ((_asset getVariable "BIS_WL_nextRearm") <= WL_SYNCED_TIME) then {
									...
									if (_asset isKindOf "Helicopter") then {  _asset setVariable ["BIS_WL_nextRearm", WL_SYNCED_TIME + WL_MAINTENANCE_COOLDOWN_REARM_Helicopter]; 
										...
										} else { if (_asset isKindOf "CUP_B_M270_HE_USMC") then {  _asset setVariable ["BIS_WL_nextRearm", WL_SYNCED_TIME + WL_MAINTENANCE_COOLDOWN_REARM_Artillery];
											} else { if (_asset isKindOf "CUP_O_BM21_RU") then {  _asset setVariable ["BIS_WL_nextRearm", WL_SYNCED_TIME + WL_MAINTENANCE_COOLDOWN_REARM_Artillery];
												} else { if (_asset isKindOf "CUP_B_M252_USMC") then {  _asset setVariable ["BIS_WL_nextRearm", WL_SYNCED_TIME + WL_MAINTENANCE_COOLDOWN_REARM_Artillery];
													} else { if (_asset isKindOf "CUP_B_M119_USMC") then {  _asset setVariable ["BIS_WL_nextRearm", WL_SYNCED_TIME + WL_MAINTENANCE_COOLDOWN_REARM_Artillery];
														} else { if (_asset isKindOf "CUP_O_2b14_82mm_RU") then {  _asset setVariable ["BIS_WL_nextRearm", WL_SYNCED_TIME + WL_MAINTENANCE_COOLDOWN_REARM_Artillery];
															} else { if (_asset isKindOf "CUP_O_D30_RU") then {  _asset setVariable ["BIS_WL_nextRearm", WL_SYNCED_TIME + WL_MAINTENANCE_COOLDOWN_REARM_Artillery];
