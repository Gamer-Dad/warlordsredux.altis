...
if (isServer) then {
	_sector spawn {
		while {!BIS_WL_missionEnd} do {
			waitUntil {sleep WL_TIMEOUT_STANDARD; BIS_WL_competingSides findIf {(_this getVariable [format ["BIS_WL_lastScanEnd_%1", _x], -9999]) > serverTime} != -1};
			...
			waitUntil {sleep WL_TIMEOUT_SHORT; BIS_WL_competingSides findIf {(_this getVariable [format ["BIS_WL_lastScanEnd_%1", _x], -9999]) > serverTime} == -1};
...
if !(isDedicated) then {
	_sector spawn {
		while {!BIS_WL_missionEnd} do {
			waitUntil {sleep WL_TIMEOUT_STANDARD; (_this getVariable [format ["BIS_WL_lastScanEnd_%1", BIS_WL_playerSide], -9999]) > serverTime && !isNull (_this getVariable ["BIS_WL_revealTrigger", objNull])};
