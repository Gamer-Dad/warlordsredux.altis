...
BIS_fnc_arsenal_data set [3, (getArray (missionConfigFile >> "arsenalConfig" >> str (side group player) >> "Uniforms"))];
BIS_fnc_arsenal_data set [4, (getArray (missionConfigFile >> "arsenalConfig" >> str (side group player) >> "Vests"))];
BIS_fnc_arsenal_data set [5, (getArray (missionConfigFile >> "arsenalConfig" >> str (side group player) >> "Backpacks"))];
BIS_fnc_arsenal_data set [6, (getArray (missionConfigFile >> "arsenalConfig" >> str (side group player) >> "Helmets"))];
BIS_fnc_arsenal_data set [11, (getArray (missionConfigFile >> "arsenalConfig" >> str (side group player) >> "Terminals"))];
