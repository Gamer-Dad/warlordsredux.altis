...
_asset setUserActionText [_actionID, format ["<t color = '%1'>%2</t>", if (_asset getVariable "bettyEnabled" == false) then {"#4bff58"} else {"#ff4b4b"}, if (_asset getVariable "bettyEnabled" == false) then {"Enable Rita"} else {"Disable Rita"}]];