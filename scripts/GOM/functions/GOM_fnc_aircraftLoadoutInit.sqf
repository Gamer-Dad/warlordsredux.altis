...
GOM_fnc_updateDialog = {
	...
	if (lbCursel 1500 < 0) exitWith {
		...
		_fueltext = if (GOM_fnc_aircraftLoadout_NeedsFuelSource && {!_canRefuel}) then {"You need fuel sources to refuel the aircraft."} else {""};
		_repairtext = if (GOM_fnc_aircraftLoadout_NeedsRepairSource && {!_canRepair}) then {"You need repair sources to repair the aircraft."} else {""};
		_rearmtext = if (GOM_fnc_aircraftLoadout_NeedsAmmoSource && {!_canRearm}) then {"You need ammo sources to rearm the aircraft."} else {""};
		...
		if (GOM_fnc_aircraftLoadout_NeedsFuelSource && {_canRefuel}) then {
			_t = "";
		...
		if (GOM_fnc_aircraftLoadout_NeedsRepairSource && {_canRepair}) then {
			_t = "";
		...
		if (GOM_fnc_aircraftLoadout_NeedsAmmoSource && {_canRearm}) then {
			_t = "";
	...
	if (lbCursel 1502 < 0 AND lbCursel 1501 >= 0) exitWith {
		...
		_priorities = 	_veh getVariable ["GOM_fnc_pylonPriorities",[]];
		{
			...
			if (count _pylons <= 6) then {
				_setAlign = "<t align='left'>";
				_break = "<br />";
			};
	...
	_mag = "N/A";
	_get = "";
	if (lbcursel 1502 > -1) then {
		_get = (lbdata [1502,(lbCursel 1502)]);
	};
	_ind2 = "N/A";
	...
	_killtext = if (_kills isequalto []) then {"Confirmed kills:<br />None"} else {
		...
		_index = -1;
		_killText = _kills apply {_index = _index + 1;_kind = ([_typeNamesS select _index,_typeNamesPL select _index] select (_x > 1));if (_x > 0) then {_out = _out + (format ["%1 %2, ",_x,_kind])};};
...
GOM_fnc_installPylons = {
	...
	[_veh, [_pylonNum, _mag, true, _pylonOwner]] remoteexec ["setPylonLoadOut", -2];
...
GOM_fnc_clearAllPylons = {
	...
	_pylonWeapons = [];
	{ 
		_pylonWeapons append getArray (_x >> "weapons") 
	} forEach ([_veh, configNull] call BIS_fnc_getTurrets);
	{ 
		_veh removeWeaponGlobal _x;
	} forEach ((weapons _veh) - _pylonWeapons);
...
GOM_fnc_setPylonsRepair = {
	...
	_repair = [_veh,_repairSource,_obj] spawn {
		...
		while {damage _veh > 0 AND alive _veh AND !_abort AND !_empty} do {
			...
			_timeout = time + 1;
			waituntil {sleep 0.1; time > _timeout || speed _veh > 3 || speed _repairSource > 3};
			_timer = _timer + 1;
		};
...
if (_leaking AND _leakingLevel < 0.9 AND _leakingLevel > 0.1) then {systemchat format ["This aircraft will leak fuel down to %1%2 when refueling!",round ((1 - _leakinglevel) * 100),"%"];systemchat "Repair it first!";playsound "Simulation_Restart"};
	...
	_refuel = [_veh,_refuelSource,_obj] spawn {
		...
		while {fuel _veh < 0.99 && {alive _veh && {!_abort && {!_empty}}}} do {
			...
			_timeout = time + 1;
			waituntil {sleep 0.; time > _timeout || speed _veh > 3 || speed _refuelSource > 3};
			_timer = _timer + 1;
		};
...
GOM_fnc_landingsCheck = {
	...
	waituntil {
		sleep 3;
		if (alive _plane && {istouchingground _plane} && {speed _plane < 100}) then {
			_check = _check + 1
		} else {
			_check = 0
		};
...
GOM_fnc_aircraftGetSerialNumber = {
	...
	if (_numberTextures isequalto [] AND lbcursel 1501 < 0 AND lbcursel 1502 < 0) exitWith {ctrlSetText [1400,"N/A"]; "N/A"};
...
GOM_fnc_aircraftSetSerialNumber = {
	...
	if (_numberTextures isequalto []) exitWith {
		...
		systemchat "Aircraft does not support tail numbers.";playsound "Simulation_Fatal"; 
		false;
	};
...
GOM_fnc_aircraftLoadoutResourcesCheck = {
	params ["_obj"];
	_nearbyVehs = (_obj nearEntities ["All", 50]) select {speed _x < 1 && {alive _x}};
	_refuelVehs = _nearbyVehs select {_x getVariable ["GOM_fnc_fuelcargo",-1] >= 0 || getNumber (configfile >> "CfgVehicles" >> typeof _x >> "transportFuel") > 0};
...
GOM_fnc_aircraftLoadout = {
	...
	findDisplay 66 displayAddEventHandler ["KeyUp",{finddisplay 66 setVariable ["GOM_fnc_keyDown",[]];
		if (_this select 3) then {
			_veh = call compile lbdata [1500,lbcursel 1500];
			_priorities = _veh getVariable ["GOM_fnc_pylonPriorities",[]];
			if (lbcursel 1501 >= 0) then {
	...
	_color = [0,0,0,0.6];
	_dark = [1100,1103,1104,1105,1109,1101,1102,1103,1104,1105,1400,1500,1501,1800,1801,1802,1803,1804,1805,1806,1807,1808,2100];
	{
