if !(isDedicated) then {
	...
	_victory = if (!isNil {(missionNamespace getVariable "BIS_WL_ffTeam")}) then {!((missionNamespace getVariable ["BIS_WL_ffTeam", Independent]) == BIS_WL_playerSide)} else {(BIS_WL_base1 getVariable "BIS_WL_owner") == BIS_WL_playerSide};
	private _audio = if (_victory) then {"Victory"} else {"Defeat"};
	_audio call WL2_fnc_announcer;
